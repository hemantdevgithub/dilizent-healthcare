...
import userAuth from "../../hooks/useAuth.js";
...
const Login = () => {
  ...
  const onSubmit = useCallback(async (data) => {
    ...
    try {
      ...
      if (
        loggedInData?.status === 201 ||
        (200 && loggedInData?.statusText === "OK")
      ) {
  ...
  return (
    <div className="grid grid-cols-1  xl:grid-cols-[69%_29%] w-full h-full mb-32 mt-14 gap-4 overflow-x-hidden ">
      {/* blue background */}
      <div className="w-full h-full hidden lg:inline-block  max-w-[2520px] ml-auto bg-blue-500 rounded-r-2xl "></div>
      ...
      <div className="flex  flex-col justify-center px-6 py-12 lg:px-8 border-[2px] border-gray-300 rounded-2xl mx-6 lg:mx-12 xl:mx-0">
        <div className=" flex flex-col justify-center w-full ">
          ...
          <form className="space-y-6 " onSubmit={handleSubmit(onSubmit)}>
            ...
            transition-all focus-visible:outline-offset-2 focus-visible:outline-rose-600 ${
          ...
          <p className="mt-6 text-center text-sm text-gray-500">
            ...
            <NavLink
              to="/Signup"
              className={`font-semibold leading-6 cursor-pointer text-rose-600 hover:text-rose-500 ${
                isLoading && "cursor-not-allowed"
              }`}
            >
              SignUp Now
